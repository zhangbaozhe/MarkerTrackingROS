...
IS_ON_DELIVER = False
IS_REACHED = False
## IMPORTANT!!! REMEBER TO UPDATE THE TWO BELOW AT THE SAME TIME
...
def initTarget(marker):
    ...
    Args:
        marker (AlvarMarker):
    """
    global TARGET_ID_SEQUENCE, TARGET_SEQUENCE, IS_ON_DELIVER
...
def isApproachOK(x, y, z):
    ...
    Returns:
        bool: _description_
    """
    if MIN_STOP_DIS <= sqrt(x**2 + y**2 + z**2) <= MIN_STOP_DIS + 0.5:
        return True
    else:
        return False
...
def markersCallBack(markersInCamera):
    ...
    global IS_ON_DELIVER, IS_ON_EXPLORE, IS_STOP, IS_REACHED
    ...
    if isApproachOK(delta_x, delta_y, delta_z) and IS_ON_DELIVER and not IS_REACHED:
        IS_ON_DELIVER = False
        IS_ON_EXPLORE = True
        IS_REACHED = True
        print("HERE1")
    # if IS_ON_EXPLORE and MIN_STOP_DIS < sqrt(delta_x**2 + delta_y**2 + delta_z**2) < MIN_DELIVER_DIS:
    if IS_REACHED and IS_ON_EXPLORE and sqrt(delta_x**2 + delta_y**2 + delta_z**2) > MIN_DELIVER_DIS:
        IS_REACHED = False
        print("THERE1")
    if not IS_REACHED and IS_ON_EXPLORE and sqrt(delta_x**2 + delta_y**2 + delta_z**2) < MIN_DELIVER_DIS: 
        IS_ON_DELIVER = True
        IS_ON_EXPLORE = False
    ...
    for marker in markersInCamera.markers:
        # print(marker.id)
        if marker.id == STOP_ID and len(TARGET_ID_SEQUENCE) == 5 and not IS_ON_DELIVER:
            IS_STOP = True
        # this is the target
...
def timerGoalCallBack(event):
    ...
    if IS_ON_DELIVER: 
        print("++++++++++++++++++ DELIVER")
        moveBaseActionClient.send_goal_and_wait(
            goal, 
            rospy.Duration(2.1) 
        )
    if IS_ON_EXPLORE:
...
def main():
    ...
    rospy.Timer(rospy.Duration(2), timerGoalCallBack)
    rospy.spin()
